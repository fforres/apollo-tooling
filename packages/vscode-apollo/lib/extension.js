...
function activate(context) {
  const serverModule = context.asAbsolutePath(
    path_1.join("node_modules/apollo-language-server/lib", "server.js")
  );
  client = languageServerClient_1.getLanguageServerClient(
    serverModule,
    outputChannel
  );
  ...
  var serverDebugMessage = new vscode_languageclient_1.NotificationType(
    "serverDebugMessage"
  );
  client.onReady().then(() => {
    ...
    const updateDecorations = () => {
      if (vscode_1.window.activeTextEditor && latestDecs) {
        ...
        vscode_1.window.activeTextEditor.setDecorations(
          engineDecoration,
          decorations
        );
      }
