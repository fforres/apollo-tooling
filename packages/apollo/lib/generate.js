...
const serializeToJSON_1 = __importDefault(
  require("apollo-codegen-core/lib/serializeToJSON")
);
...
const promisedReadFile = util_1.default.promisify(localfs_1.fs.readFile);
const promisedWriteFile = util_1.default.promisify(localfs_1.fs.writeFile);
function toPath(uri) {
...
async function generate(
  ...
  if (
    ...
    !localfs_1.fs.existsSync(outputPath)
  ) {
  ...
  if (target === "swift") {
    ...
    const generator = apollo_codegen_swift_1.generateSource(
      context,
    ...
    const context = compiler_1.compileToIR(schema, document, options);
    const { generatedFiles, common } = apollo_codegen_flow_1.generateSource(
      context
    );
    const outFiles = {};
    if (nextToSources) {
      generatedFiles.forEach(({ sourcePath, fileName, content }) => {
        const dir = path_1.default.join(
          path_1.default.dirname(
            path_1.default.posix.relative(rootPath, toPath(sourcePath))
          ),
          outputPath
        );
      ...
      await writeGeneratedFiles(outFiles, path_1.default.resolve("."));
    ...
    const context = compiler_1.compileToIR(schema, document, options);
    const generatedFiles = apollo_codegen_typescript_1.generateLocalSource(
      context
    );
    const generatedGlobalFile = apollo_codegen_typescript_1.generateGlobalSource(
      context
    );
    const outFiles = {};
    if (
      ...
      generatedFiles.forEach(({ sourcePath, fileName, content }) => {
        ...
        if (nextToSources) {
          dir = path_1.default.join(
            path_1.default.dirname(
              path_1.default.relative(rootPath, toPath(sourcePath))
            ),
            dir
          );
          if (!localfs_1.fs.existsSync(dir)) {
    ...
    let output;
    const context = legacyIR_1.compileToLegacyIR(
      schema,
      document,
...
function writeGeneratedFiles(generatedFiles, outputDirectory, terminator = "") {
  return p_map_1.default(
    Object.entries(generatedFiles),
    async ([fileName, generatedFile]) => {
      const finalFileName = path_1.default.join(outputDirectory, fileName);
      ...
      promisedWriteFile(path_1.default.join(finalFileName), newFileContent);
    },
    {
      concurrency: 10
    }
