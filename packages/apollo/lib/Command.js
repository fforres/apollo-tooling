...
class ProjectCommand extends command_1.default {
  ...
  async createConfig(flags) {
    const service = flags.key
      ? apollo_language_server_1.getServiceFromKey(flags.key)
      : undefined;
    const config = await apollo_language_server_1.loadConfig({
      configPath:
        flags.config && path_1.parse(path_1.resolve(flags.config)).dir,
    ...
    config.graph =
      flags.graph ||
      apollo_language_server_1.getGraphIdFromConfig(config.rawConfig);
    if (flags.tag) {
      console.warn(
