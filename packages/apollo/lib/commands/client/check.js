...
class ClientCheck extends Command_1.ClientCommand {
  constructor() {
    ...
    this.logMessagesForOperation = ({ validationResults, operation }) => {
      ...
      this.log(
        `${name}: ${chalk_1.default.cyan(
          `${relativePath}:${locationOffset.line}`
        )}\n`
      );
    ...
    this.printStats = (validationResults, operations) => {
      ...
      if (counts.failure > 0) {
        this.log(
          chalk_1.default.red(
            `${counts.failure} failure${counts.failure > 1 ? "s" : ""}`
          )
        );
      }
      if (counts.warning > 0) {
      ...
      ({ project, config }) => [
        {
          ...
          task: async ctx => {
            ...
            ctx.operations = Object.entries(
              ...
              name,
              relativePath: path_1.relative(
                config.configURI ? config.configURI.fsPath : "",
                vscode_uri_1.default.parse(doc.definitions[0].loc.source.name)
                  .fsPath
              ),
