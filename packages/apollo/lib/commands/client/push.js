  ...
  async run() {
    ...
    try {
      await this.runTasks(({ flags, project, config }) => {
        const clientBundleInfo = `${chalk_1.default.cyan(
        ...
        return [
          {
            ...
            task: async (ctx, task) => {
              const operationManifest = getOperationManifestFromProject_1.getOperationManifestFromProject(
                this.project
              );
              ctx.operationManifest = operationManifest;
              task.title = `Extracted ${utils_1.pluralize(
          ...
          {
            title: `Checked operations against ${chalk_1.default.cyan(
          ...
          },
          {
            ...
            task: async (_, task) => {
              ...
              const { operations: _op } = variables,
                restVariables = __rest(variables, ["operations"]);
              ...
              if (!registrationSuccess) {
                if (invalidOperations) {
                  ...
                  task.title = `Failed to push operations, due to ${utils_1.pluralize(
                ...
                if (newOperations && newOperations.length) {
                  task.title = `Successfully pushed ${utils_1.pluralize(
      ...
      ([operationName, document]) => {
        ...
        const relativePath =
          ...
          path_1.relative(
            ...
            vscode_uri_1.default.parse(operationDefinition.loc.source.name)
              .fsPath
          );
        const line =
          operationDefinition &&
        ...
        return [
          apollo_graphql_1.operationHash(
            apollo_graphql_1.defaultOperationRegistrySignature(
              document,
              operationName
            )
          ),
