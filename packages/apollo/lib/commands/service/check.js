...
function formatMarkdown({
  ...
  if (validationConfig) {
    ...
    validationText = `🔢 Compared **${utils_1.pluralize(
      diffToPrevious.changes.length,
      "schema change"
    )}** against **${utils_1.pluralize(
...
${
  breakingChanges.length > 0
    ? `❌ Found **${utils_1.pluralize(
        diffToPrevious.changes.filter(change => change.severity === "FAILURE")
          .length,
        "breaking change"
      )}** that would affect **${utils_1.pluralize(
        affectedQueryCount,
        "operation"
      )}** across **${utils_1.pluralize(
...
function formatCompositionErrorsMarkdown({
  compositionErrors,
  graphName,
  serviceName,
  tag
...
function formatHumanReadable({ checkSchemaResult, graphCompositionID }) {
  ...
  if (changes.length === 0) {
    ...
    const breakingChanges = sortedChanges.filter(
      change => change.severity === graphqlTypes_1.ChangeSeverity.FAILURE
    );
      ...
      await this.runTasks(
        ({ config, flags, project }) => {
          ...
          return [
            {
              enabled: () => !!serviceName,
              title: `Validate graph composition for service ${chalk_1.default.cyan(
                serviceName || ""
              )} on graph ${chalk_1.default.cyan(graphSpecifier)}`,
              task: async (ctx, task) => {
                if (!serviceName) {
                  throw new Error(
                ...
                task.output = `Attempting to compose graph with ${chalk_1.default.cyan(
                  serviceName
                ...
                const {
                  ...
                  Object.assign(
                    ...
                    {
                      gitContext: Object.assign(
                        Object.assign(
                          Object.assign({}, gitInfoFromEnv),
                ...
                task.title = `Found ${utils_1.pluralize(
                  compositionValidationResult.errors.length,
                  "graph composition error"
                )} for service ${chalk_1.default.cyan(
                  serviceName
                )} on graph ${chalk_1.default.cyan(graphSpecifier)}`;
            ...
            },
            {
              ...
              }schema against metrics from variant ${chalk_1.default.cyan(
                graphVariant
              )} on graph ${chalk_1.default.cyan(graphSpecifier)}`,
              enabled: () => !serviceName,
              task: async (ctx, task) => {
                ...
                const { schema: _ } = variables,
                  restVariables = __rest(variables, ["schema"]);
                ...
                const hours = validationConfig
                  ? Math.abs(
                      moment_1
                        ...
                        .add(validationConfig.from, "second")
                        .diff(
                          moment_1.default().add(validationConfig.to, "second"),
                          "hours"
                        )
                    )
                  : null;
                task.title = `Compared ${utils_1.pluralize(
                  chalk_1.default.cyan(schemaChanges.length.toString()),
                  "schema change"
                )} against ${utils_1.pluralize(
                  chalk_1.default.cyan(numberOfCheckedOperations.toString()),
                  "operation"
                )}${
                  hours
                    ? ` over the last ${chalk_1.default.cyan(
            ...
            },
            {
              ...
              task: async (ctx, task) => {
                const breakingSchemaChangeCount = ctx.checkSchemaResult.diffToPrevious.changes.filter(
                  change =>
                    change.severity === graphqlTypes_1.ChangeSeverity.FAILURE
                ).length;
                const nonBreakingSchemaChangeCount =
                  ctx.checkSchemaResult.diffToPrevious.changes.length -
                  breakingSchemaChangeCount;
                task.title = `Found ${utils_1.pluralize(
                  chalk_1.default.cyan(breakingSchemaChangeCount.toString()),
                  "breaking change"
                )} and ${utils_1.pluralize(
                  chalk_1.default.cyan(nonBreakingSchemaChangeCount.toString()),
                  "compatible change"
                )}`;
    ...
    if (compositionErrors) {
      ...
      if (unformattedErrors.length)
        this.log(
          table_1.table([
            ["Message"],
            ...unformattedErrors.map(e => [e.message])
          ])
        );
      ...
      if (
        checkSchemaResult.diffToPrevious.changes.find(
          ({ severity }) => severity === graphqlTypes_1.ChangeSeverity.FAILURE
        )
      ) {
        if (shouldAlwaysExit0) {
          return;
...
ServiceCheck.description =
  "Check a service against known operation workloads to find breaking changes";
ServiceCheck.flags = Object.assign(
  Object.assign({}, Command_1.ProjectCommand.flags),
  {
