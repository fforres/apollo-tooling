...
const formatImplementingService = (
  ...
  return {
    ...
    updatedAt: `${moment_1
      .default(implementingService.updatedAt)
      .format("D MMMM YYYY")} (${moment_1
      .default(implementingService.updatedAt)
      .from(effectiveDate)})`
  };
};
function formatHumanReadable({
  ...
  if (
    ...
    const sortedImplementingServices = lodash_sortby_1.default(
  ...
  async run() {
    ...
    try {
      await this.runTasks(({ config, flags, project }) => {
        ...
        return [
          {
            title: `Fetching list of services for graph ${chalk_1.default.cyan(
...
ServiceList.flags = Object.assign(
