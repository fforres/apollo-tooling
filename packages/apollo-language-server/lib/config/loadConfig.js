...
async function loadConfig({
  ...
  envFileNames.forEach(envFile => {
    ...
    if (fs_1.existsSync(dotEnvPath) && fs_1.lstatSync(dotEnvPath).isFile()) {
      ...
      if (legacyKey && key) {
        utilities_1.Debug.warning(
          `Both ${exports.legacyKeyEnvVar} and ${exports.keyEnvVar} were found. ${exports.keyEnvVar} will take precedence.`
        );
      }
      if (legacyKey) {
        utilities_1.Debug.warning(
          `[Deprecation warning] Setting the key via ${exports.legacyKeyEnvVar} is deprecated and will not be supported in future versions. Please use ${exports.keyEnvVar} instead.`
        );
      }
