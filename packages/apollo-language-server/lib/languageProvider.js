...
function locationForASTNode(node) {
  ...
  if (!uri) return null;
  return vscode_languageserver_1.Location.create(
    uri,
    source_1.rangeForASTNode(node)
  );
}
...
class GraphQLLanguageProvider {
  ...
  async provideCompletionItems(uri, position, _token) {
    ...
    const positionInDocument = source_1.positionFromPositionInContainingDocument(
      document.source,
      position
    );
    const token = getAutocompleteSuggestions_1.getTokenAtPosition(
      document.source.body,
      positionInDocument
    );
    const state =
      token.state.kind === "Invalid" ? token.state.prevState : token.state;
    const typeInfo = getAutocompleteSuggestions_1.getTypeInfo(
      project.schema,
      token.state
    );
    ...
    const suggestions = graphql_language_service_interface_1.getAutocompleteSuggestions(
      project.schema,
      document.source.body,
      positionInDocument
    );
    if (
      ...
      if (graphql_1.isAbstractType(parentType)) {
        parentFields[graphql_1.TypeNameMetaFieldDef.name] =
          graphql_1.TypeNameMetaFieldDef;
      }
      if (parentType === project.schema.getQueryType()) {
        parentFields[graphql_1.SchemaMetaFieldDef.name] =
          graphql_1.SchemaMetaFieldDef;
        parentFields[graphql_1.TypeMetaFieldDef.name] =
          graphql_1.TypeMetaFieldDef;
      }
      return suggestions.map(suggest => {
        ...
        if (!suggestedField) {
          ...
          const requiredArgs = suggestedField.args.filter(a =>
            graphql_1.isNonNullType(a.type)
          );
          const paramsSection =
          ...
          const snippet = hasFields(suggestedField.type)
            ...
            : `${suggest.label}${paramsSection}${directives}`;
          return Object.assign(Object.assign({}, suggest), {
            insertText: snippet,
  ...
  async provideHover(uri, position, _token) {
    ...
    const nodeAndTypeInfo = source_1.getASTNodeAndTypeInfoAtPosition(
      document.source,
      positionInDocument,
      document.ast,
      project.schema
        ...
        case graphql_1.Kind.FIELD: {
          ...
          if (parentType && fieldDef) {
            ...
            return {
              ...
              range: source_1.rangeForASTNode(
                graphql_2.highlightNodeForNode(node)
              )
            };
          }
          break;
    ...
    if (nodeAndTypeInfo) {
      ...
      switch (node.kind) {
        case graphql_1.Kind.FRAGMENT_DEFINITION: {
          if (!client_1.isClientProject(project)) return null;
          const fragmentName = node.name.value;
          return project
        ...
        case graphql_1.Kind.FIELD_DEFINITION: {
          if (!client_1.isClientProject(project)) return null;
          const offset = source_1.positionToOffset(
            document.source,
            positionInDocument
          );
          let parent = null;
    ...
    for (const definition of definitions) {
      if (graphql_1.isExecutableDefinitionNode(definition)) {
        ...
        symbols.push({
          ...
          range: source_1.rangeForASTNode(definition),
          selectionRange: source_1.rangeForASTNode(
            graphql_2.highlightNodeForNode(definition)
          )
        });
      } else if (
  ...
  async provideCodeAction(uri, range, _token) {
    ...
    for (const [
      ...
      for (const diagnostic of diagnostics) {
        if (
          diagnostics_1.GraphQLDiagnostic.is(diagnostic) &&
    ...
    for (const error of errors) {
      ...
      const codeAction = vscode_languageserver_1.CodeAction.create(
        message,
        { changes: { [uri]: edits } },
