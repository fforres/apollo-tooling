...
function getFieldDef(schema, parentType, fieldAST) {
  ...
  if (
    name === graphql_1.SchemaMetaFieldDef.name &&
    schema.getQueryType() === parentType
  ) {
    return graphql_1.SchemaMetaFieldDef;
  }
  if (
    name === graphql_1.TypeMetaFieldDef.name &&
    schema.getQueryType() === parentType
  ) {
...
function removeOrphanedFragmentDefinitions(
  ast,
...
function withTypenameFieldAddedWhereNeeded(ast) {
  return graphql_1.visit(ast, {
    ...
    leave(node) {
      if (
        !(
          node.kind === graphql_1.Kind.FIELD ||
