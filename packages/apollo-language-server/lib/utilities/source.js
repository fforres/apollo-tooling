...
function positionFromPositionInContainingDocument(source, position) {
  ...
  return vscode_languageserver_1.Position.create(
...
function rangeInContainingDocument(source, range) {
  ...
  return vscode_languageserver_1.Range.create(
...
function rangeForASTNode(node) {
  ...
  return vscode_languageserver_1.Range.create(
    positionFromSourceLocation(
      source,
      location_1.getLocation(source, location.start)
    ),
    positionFromSourceLocation(
      source,
      location_1.getLocation(source, location.end)
    )
  );
}
