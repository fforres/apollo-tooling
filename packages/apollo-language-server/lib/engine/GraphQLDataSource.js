...
class GraphQLDataSource {
  ...
  async executeSingleOperation(operation) {
    ...
    const [error, response] = await await_to_js_1.default(
      apollo_link_1.makePromise(apollo_link_1.execute(link, operation))
    );
    if (error) {
  ...
  resolveUri() {
    ...
    if (!baseURL) {
      throw new apollo_server_errors_1.ApolloError(
