...
function collectExecutableDefinitionDiagnositics(
  schema,
  queryDocument,
  fragments = {},
  rules
  ...
  const diagnostics = [];
  for (const error of validation_1.getValidationErrors(
    schema,
  ...
  for (const error of graphql_1.findDeprecatedUsages(
    schema,
    astWithExecutableDefinitions
  )) {
    diagnostics.push(
  ...
  return error.nodes.map(node => {
    return {
      ...
      range: source_1.rangeForASTNode(
        graphql_2.highlightNodeForNode(node) || node
      ),
      error
    };
  });
...
(function(GraphQLDiagnostic) {
  ...
  (GraphQLDiagnostic =
    exports.GraphQLDiagnostic || (exports.GraphQLDiagnostic = {}))
);
class DiagnosticSet {
  constructor() {
